...
async function setWeatherInformation() {
  await fetch(
    ...
    .then((r) => {
      ...
      if (!description || !icon || !temp || !sunrise || !sunset)
        throw new Error("some data is undefind");
      ...
      const sun_rise = new Date(sunrise * 1000).toLocaleString(
        "en-GB",
        timezonePreset
      );
      const sun_set = new Date(sunset * 1000).toLocaleString(
        "en-GB",
        timezonePreset
      );
...
async function setInstagramPosts() {
  try {
    const instagramImages =
      await puppeteerService.getLatestInstagramPostsFromAccount("israel", 3);
